...
[ ] `filename`이 `.pdf`로 끝나며 실제 파일명과 일치하는가?
[ ] `keywords` 배열이 **정확히 3개**인가?
[ ] 각 키워드가 task 범주(Bug Detection, Code Generation 등)가 아닌 **technical term**인가?
[ ] `research_type`이 5가지 허용값 중 하나인가?
