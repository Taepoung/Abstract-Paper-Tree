...
- 스크립트는 **현재 작업 디렉토리(논문 디렉토리) 기준**으로 동작한다. 인자 없이 실행한다.
  - 예외: 재귀 클러스터링(5-3)에서 서브 디렉토리를 처리할 때는 절대경로를 인자로 전달한다.
...
```
사용법: python skills/APT/scripts/scan_papers.py
...
```
사용법: python skills/APT/scripts/merge_results.py
...
```
사용법: python skills/APT/scripts/extract_subcluster.py
...
```
사용법: python skills/APT/scripts/build_all.py
...
```bash
python skills/APT/scripts/scan_papers.py
```
...
```bash
python skills/APT/scripts/merge_results.py
```
...
```bash
python skills/APT/scripts/extract_subcluster.py
```
...
1. 해당 디렉토리의 `results.jsonl`을 Read로 읽습니다. (경로: `clusters/{safe_name}/results.jsonl`)
2. 논문을 클러스터로 분류합니다.
3. 분류 기준에 맞는 JSON만 Write로 저장합니다:
   - `problem.json` 계열 → `clusters/{safe_name}/problem.json`
   - `method.json` 계열 → `clusters/{safe_name}/method.json`

**논문 중복 허용**: 한 논문이 여러 클러스터에 동시에 속할 수 있다. 트리에서 중복 리프끼리 연결선으로 표시된다.

**5-3. 재귀**

`depth - 1`이 1보다 크다면, 5-1의 stdout에 출력된 각 서브 디렉토리 절대경로에 대해 5단계를 반복합니다.

서브 디렉토리는 절대경로를 인자로 전달합니다:

```bash
python skills/APT/scripts/extract_subcluster.py {sub_dir_절대경로}
```

이 스크립트도 stdout에 생성된 하위 서브 디렉토리 절대경로를 출력합니다. `depth`는 `depth - 1`로 줄이고, 분류 기준은 그대로 유지합니다.
...
```bash
python skills/APT/scripts/build_all.py
```
